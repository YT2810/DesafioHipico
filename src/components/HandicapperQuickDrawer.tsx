...
export interface TickerEntry {
  ...
  // Sponsor / promo slot fields (type defaults to 'handicapper' when absent)
  slotType?: 'handicapper' | 'sponsor' | 'promo';
  sublabel?: string;
  badgeText?: string;
  accentColor?: string;
  logoUrl?: string;
...
export default function HandicapperQuickDrawer({ entry, onClose, meetingId }: DrawerProps) {
  ...
  const waLink = !isSponsor && entry.contactNumber
    ...
    <>
      ...
      <div className="fixed inset-x-0 bottom-0 z-50 md:inset-x-auto md:right-0 md:top-0 md:bottom-0 md:w-[360px] flex flex-col bg-gray-950 border-t border-gray-800 md:border-t-0 md:border-l md:border-gray-800 rounded-t-2xl md:rounded-none shadow-2xl overflow-hidden animate-slide-up md:animate-slide-left">
        ...
        <div className="flex items-center gap-3 px-5 pt-3 pb-4 md:pt-5 border-b border-gray-800 shrink-0">
          <div className="w-12 h-12 rounded-2xl flex items-center justify-center text-lg font-black text-black shrink-0 overflow-hidden"
            style={{ backgroundColor: accent }}>
            {entry.logoUrl
              ? <img src={entry.logoUrl} alt={entry.pseudonym} className="w-full h-full object-cover" />
              : <span className="font-black text-black">{entry.pseudonym[0].toUpperCase()}</span>
            }
          </div>
          <div className="flex-1 min-w-0">
            ...
            <p className="text-xs text-gray-500 mt-0.5">
              {isSponsor
                ? (entry.sublabel ?? (entry.slotType === 'promo' ? '📢 Promoción de la plataforma' : '🤝 Patrocinador'))
            ...
            <div className="space-y-4">
              {entry.sublabel && (
                <div className="bg-gray-900 border border-gray-800 rounded-2xl px-4 py-4 text-center">
                  <p className="text-sm text-gray-300 leading-relaxed">{entry.sublabel}</p>
                </div>
              )}
              {entry.actionUrl && (
                <a
                  href={entry.actionUrl}
                  target={entry.actionUrl.startsWith('http') ? '_blank' : undefined}
                  rel="noopener noreferrer"
                  onClick={onClose}
                  className="flex items-center justify-center gap-2 w-full py-4 rounded-2xl text-black font-extrabold text-base transition-all active:scale-95"
                  style={{ backgroundColor: accent }}
                >
                  {entry.actionLabel ?? 'Ver más →'}
                </a>
              )}
              <button
                onClick={onClose}
                className="w-full py-3 rounded-2xl text-sm font-semibold text-gray-400 bg-gray-900 border border-gray-800 hover:bg-gray-800 transition-colors"
              >
                Volver a los pronósticos
              </button>
            </div>
          ) : (
            /* ── HANDICAPPER view ── */
            <>
              ...
              {entry.totalRaces >= 5 ? (
                <div>
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    ...
                    <div className="bg-gray-900 border border-gray-800 rounded-2xl px-4 py-3 flex flex-col items-center gap-0.5">
                      <span className="text-2xl font-black text-white">{entry.eGeneral}%</span>
                      <span className="text-xs text-gray-500">E-General</span>
                    </div>
                  </div>
                  <p className="text-xs text-gray-700 text-center mt-1.5">{entry.totalRaces} carreras evaluadas</p>
                </div>
              ) : (
                <div className="bg-gray-900 border border-gray-800 rounded-2xl px-4 py-3 text-center">
                  ...
                  <p className="text-xs text-gray-700 mt-0.5">Mínimo 5 carreras con resultados</p>
                </div>
              )}

              {/* Audios VIP */}
              <div>
                <p className="text-xs text-gray-600 uppercase tracking-wider font-semibold mb-2">🎧 Audios VIP</p>
                {loadingAudio ? (
                  ...
                  <div className="bg-gray-900 border border-gray-800 rounded-xl px-4 py-3 text-center">
                    <p className="text-xs text-gray-600">Sin audios publicados hoy</p>
                  </div>
                ) : (
                  <div className="space-y-2">
                    {audios.map(audio => (
                      <div key={audio._id} className="bg-gray-900 border border-gray-800 rounded-xl px-3 py-2.5 flex items-center gap-3">
                        <div className="w-9 h-9 rounded-xl bg-gray-800 flex items-center justify-center shrink-0 text-lg">🎧</div>
                        <div className="flex-1 min-w-0">
                          <p className="text-xs font-semibold text-white truncate">{audio.title}</p>
                          <div className="flex items-center gap-2 mt-0.5">
                            {audio.durationSecs && <span className="text-[10px] text-gray-600">{fmtDuration(audio.durationSecs)}</span>}
              ...
              <div className="space-y-2">
                ...
                <Link href={`/handicapper/${entry.id}`} onClick={onClose}

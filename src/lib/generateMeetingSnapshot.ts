...
export async function generateMeetingSnapshot(meetingId: string): Promise<any> {
  await connectMongo();
  ...
  const racesOut = races.map((race: any) => {
    ...
    const entriesOut = raceEntries.map((e: any) => {
      ...
      return {
        ...
        color: (horse?.color as string | undefined) ?? null,
        sire: (horse?.pedigree?.sire as string | undefined) ?? null,
        dam: (horse?.pedigree?.dam as string | undefined) ?? null,
        gender: (horse?.gender as string | undefined) ?? null,
        horseId: horseIdStr,
  ...
  // Save snapshot to DB (upsert — overwrite if exists)
  await MeetingSnapshot.findOneAndUpdate(
    { meetingId: new Types.ObjectId(meetingId) },
    { data: result, generatedAt: new Date() },
